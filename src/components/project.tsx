...
const Project: React.FC<ProjectProps> = ({ project }) => {
  return (
    <div className="flex flex-col sm:flex-row  w-full max-w-full h-auto max-h-full gap-y-4 gap-x-4 overflow-y-scroll">
      ...
      <div className="flex flex-col gap-y-2 w-full min-h-[400px] h-auto sm:h-full  sm:max-w-[60%] overflow-y-scroll scrollbar-hide">
        <div className=" flex flex-row gap-x-2 overflow-x-scroll w-full scrollbar-hide">
          {project.images.desktop.map((url) => (
            <img
              src={`${BASE_URL}${url}`}
              className="sm:h-[250px] w-auto h-[140px] "
            />
          ))}
        </div>
        <div className=" flex flex-row gap-x-2 overflow-x-scroll w-full scrollbar-hide">
          {project.images.mobile.map((url) => (
            <img
              src={`${BASE_URL}${url}`}
              className="w-[100px] sm:w-full sm:max-w-[150px] h-auto"
            />
          ))}
        </div>
      </div>
